...
class TelaCliente(CTkToplevel):
    ...
    def __init__(self, master=None):
        ...
        self.title('Clientes')
        self.telaCadastrarCliente = None
        self.telaEditarCliente = None
        self.clientes = None
    ...
    def carregar_widgets(self):
        ...
        self.f_cliente = CTkScrollableFrame(self, corner_radius=20)
        self.f_cliente.grid_columnconfigure(0, weight=1)
        self.f_cliente.grid_columnconfigure(1, weight=3)
        self.f_cliente.grid_columnconfigure(2, weight=5)
        self.f_cliente.grid_columnconfigure(3, weight=3)
        self.f_cliente.grid_columnconfigure(4, weight=4)
        self.f_cliente.grid_columnconfigure(5, weight=5)
        self.f_cliente.grid_columnconfigure(6, weight=0)
        self.f_cliente.grid_columnconfigure(7, weight=0)

        
        
        self.pesquisa = CTkEntry(self, placeholder_text='Nome do Cliente', width=150,height=40, font=self.font_entry)
        ...
        self.f_cliente.grid(column=0, row=1, columnspan=3, sticky='wsen', padx=(10,0), ipadx=20, ipady=20)
        self.carregar_funcionarios()
             
    def carregar_funcionarios(self):
        [x.destroy() for x in self.f_cliente.winfo_children()]
        self.funcionarios = funcionarioDAO().select_all_funcionarios()
        rows = len(self.funcionarios)
        CTkLabel(self.f_cliente, text='ID', font=self.font_tabela_coluna).grid(column=0, row=0, pady=0, padx=0)
        CTkLabel(self.f_cliente, text='NOME', font=self.font_tabela_coluna).grid(column=1, row=0, pady=0,padx=0)
        CTkLabel(self.f_cliente, text='SOBRENOME', font=self.font_tabela_coluna).grid(column=2, row=0, pady=0, padx=0)
        CTkLabel(self.f_cliente, text='CPF', font=self.font_tabela_coluna).grid(column=3, row=0, pady=0, padx=0)
        CTkLabel(self.f_cliente, text='CARGO', font=self.font_tabela_coluna).grid(column=4, row=0, pady=0, padx=0)
        CTkLabel(self.f_cliente, text='TELEFONE', font=self.font_tabela_coluna).grid(column=5, row=0, pady=0, padx=0)
        CTkFrame(self.f_cliente, bg_color='white', height=1).grid(column=0, columnspan=6, row=0, sticky='wes')
        CTkFrame(self.f_cliente, bg_color='white', height=1).grid(column=0, columnspan=6, row=0, sticky='wen')
        
        for l in range(rows):
            CTkLabel(self.f_cliente, text=self.funcionarios[l].id, font=self.font_tabela_linha).grid(column=0, row=l+1)
            CTkLabel(self.f_cliente, text=self.funcionarios[l].nome, font=self.font_tabela_linha).grid(column=1, row=l+1)
            CTkLabel(self.f_cliente, text=self.funcionarios[l].sobrenome, font=self.font_tabela_linha).grid(column=2, row=l+1)
            CTkLabel(self.f_cliente, text=self.funcionarios[l].get_cpf(), font=self.font_tabela_linha).grid(column=3, row=l+1)
            CTkLabel(self.f_cliente, text=self.funcionarios[l].get_cargo_nome(), font=self.font_tabela_linha).grid(column=4, row=l+1)
            CTkLabel(self.f_cliente, text=self.funcionarios[l].get_telefone(), font=self.font_tabela_linha).grid(column=5, row=l+1)
            CTkButton(self.f_cliente, text='', image=CTkImage(Image.open(img_editar), size=(32,32)), width=70, 
                      command=lambda x=l: self.abrir_telaEditarFuncionario(self.funcionarios[x])).grid(column=6, row=l+1, padx=10, pady=5)
            CTkButton(self.f_cliente, text='',image=CTkImage(Image.open(img_excluir), size=(32,32)), width=70, 
